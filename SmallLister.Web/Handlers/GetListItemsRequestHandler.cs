...
public class GetListItemsRequestHandler(
    ...
    IUserActionRepository userActionRepository) : IRequestHandler<GetListItemsRequest, GetListItemsResponse>
{
    ...
    public async Task<GetListItemsResponse> Handle(GetListItemsRequest request, CancellationToken cancellationToken)
    {
        ...
        var userLists = await userListRepository.GetListsAsync(user);
        var (overdueCount, dueCount, totalCount, totalWithDueDateCount, userListCounts) = await userListRepository.GetListCountsAsync(user);
        ...
        if (request.List != null)
        {
            if (request.List == IndexViewModel.AllList || (request.List == IndexViewModel.DueList && !hasDueItems))
            {
                ...
                await userAccountRepository.SetLastSelectedUserListIdAsync(user, null);
            }
            else if (request.List == IndexViewModel.AllWithDueDateList)
            {
                ...
                await userAccountRepository.SetLastSelectedUserListIdAsync(user, -2);
            }
            else if (request.List == IndexViewModel.DueList && hasDueItems)
            {
                ...
                await userAccountRepository.SetLastSelectedUserListIdAsync(user, -1);
            }
            ...
            else
            {
                ...
                await userAccountRepository.SetLastSelectedUserListIdAsync(user, listId);
            }
        }
        else
        {
            ...
            if (selectedList == null)
                ...
                (_, items, pageNumber, pageCount) = await GetListItemsAsync(user, userLists, lists, user.LastSelectedUserListId ?? 0, request.Sort, request.PageNumber);
        }

        var (undoAction, redoAction) = await userActionRepository.GetUndoRedoActionAsync(user);
    ...
    {
        var (userItems, resultPageNumber, pageCount) = await userItemRepository.GetItemsAsync(user, null, pageNumber: pageNumber);
    ...
    {
        var (userItems, resultPageNumber, pageCount) = await userItemRepository.GetItemsAsync(user, null, new UserItemFilter { WithDueDate = true }, pageNumber);
    ...
    {
        var (userItems, resultPageNumber, pageCount) = await userItemRepository.GetItemsAsync(user, null, new UserItemFilter { Overdue = true, DueToday = true }, pageNumber);
    ...
    private async Task<(UserListModel, IEnumerable<UserItem>, int PageNumber, int PageCount)> GetListItemsAsync(UserAccount user, IEnumerable<UserList> userLists, IEnumerable<UserListModel> lists, int listId, ItemSortOrder? sortOrder, int? pageNumber)
    {
        ...
        if (list != null && sortOrder != null)
        {
            ...
            await userItemRepository.UpdateOrderAsync(user, list, sortOrder, savedItemSortOrders);

            var savedListItemSortOrder = list.ItemSortOrder;
            list.ItemSortOrder = sortOrder;
            await userListRepository.SaveAsync(list);
            selectedList.ItemSortOrder = sortOrder;

            await userActionsService.AddAsync(user, new ReorderItemsAction(savedItemSortOrders, (list.UserListId, savedListItemSortOrder, sortOrder)));
        }

        var (userItems, resultPageNumber, pageCount) = await userItemRepository.GetItemsAsync(user, list, pageNumber: pageNumber);
