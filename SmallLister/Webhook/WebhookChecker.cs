    ...
    : IWebhookChecker
{
    public async Task CheckAsync(CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogInformation($"Got {userListWebhookQueueByUserListId.Count} UserLists in the webhook queue, sending...");
            ...
            logger.LogDebug($"Sent {webhooksSent.Count} webhooks, updating queue with payload details");
            foreach (var item in userListWebhookQueueByUserListId.SelectMany(x => x.Value))
            {
                ...
                {
                    logger.LogWarning($"Did not send webhooks for user list id {item.UserListId}, not setting user list webhook queue entry as sent");
                    continue;
                }
        ...
        {
            logger.LogInformation($"Got {userItemWebhookQueueByUserItemId.Count} UserItem in the webhook queue, sending...");
            ...
            var webhooksSent = (await webhookSender.SendAsync(webhooksToSend, WebhookType.ListItemChange)).ToLookup(s => s.WebhookToSend.ListItemId);
            ...
            foreach (var item in userItemWebhookQueueByUserItemId.SelectMany(x => x.Value))
            {
                ...
                {
                    logger.LogWarning($"Did not send webhooks for user item id {item.UserItemId}, not setting user item webhook queue entry as sent");
                    continue;
                }

                var payloadSent = webhooksSent[item.UserItemId.ToString()].FirstOrDefault().PayloadSent;
                logger.LogDebug($"Item {item.UserItemId}, sent payload: {payloadSent}");
                await webhookQueueRepository.SentAsync(item, payloadSent);
            }
    ...
    private IEnumerable<(int Key, WebhookEventType EventType)> GetWebhookEvents<T>(int key, IEnumerable<T> items, Func<T, WebhookEventType> getEventType)
    {
        ...
        if (anyNew && anyDelete)
        {
            logger.LogInformation($"Item {key} has been created & deleted - no need to send webhook");
            yield break;
        }
        else if (anyDelete)
